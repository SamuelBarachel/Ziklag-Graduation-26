...
export function FamilyScene() {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setPhase(3), 11000), // crossfade to photo 2
      setTimeout(() => setPhase(4), 19500), // begin exit
    ];
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0"
        initial={{ opacity: 0, scale: 1.04 }}
        animate={phase >= 3 ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 1.04 }}
      ...
      {/* Bottom text overlay */}
      <div className="absolute bottom-[8vh] left-0 right-0 z-20 flex flex-col items-center pb-6"
      ...
      >
        ...
        <motion.div
          ...
          initial={{ width: 0 }}
          animate={phase >= 2 ? { width: '25vw' } : { width: 0 }}
