        ...
        <div className="flex flex-row items-center justify-center gap-12 w-full">
          ...
          <div className="relative w-1/3 aspect-[3/4] flex flex-col items-center">
            ...
            <div className="mt-8 text-center absolute -bottom-32 w-[150%]">
                  ...
                  <span key={i} className="inline-block mr-3">
                    {word.split('').map((char, j) => (
                      <motion.span 
                        ...
                        transition={{ duration: 0.8, delay: phase >= 2 ? (i * 0.15) + (j * 0.03) : 0 }}
                      >
                        {char}
                      </motion.span>
                    ))}
              ...
              <motion.p 
                className="mt-4 font-body text-base md:text-lg text-[#fff8e7]/80 tracking-widest uppercase"
                initial={{ opacity: 0, y: 10 }}
                animate={phase >= 3 ? { opacity: 1, y: 0 } : { opacity: 0, y: 10 }}
                transition={{ duration: 1, ease: 'easeOut' }}
              >
                BSN, RN, PHN — Bachelor of Science in Nursing with Public Health Nursing Credential, with Honors
              </motion.p>
            </div>
          </div>
